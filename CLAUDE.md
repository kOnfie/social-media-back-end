...
Standard Nest module-per-domain layout under `src/modules/`: `auth`, `user`, `session`. Cross-cutting code lives in `src/common/` (guards, filters, http helpers, provider interfaces, utils).
...
**Entities/relations**: `User` (`src/modules/user/entities/user.entity.ts`) has `OneToMany` sessions and a self-referential `ManyToMany` follow graph (`following`/`followers`) backed by a `follows` join table (`follower_id`/`followee_id`) — no `Follow` entity/module exists yet even though the relation is defined. `synchronize: true` is enabled on the TypeORM connection (`src/app.module.ts`), so entity changes apply to the local DB schema automatically on boot — no migrations exist.
